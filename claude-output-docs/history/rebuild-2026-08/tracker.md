...
| M-1 | merge | PR #8 qari-assets MERGED (by user) | ✅→⚠️ | merge `ffe0585` | **`[skip release]` guard did NOT hold** — GitHub merge-commit subject was the PR title, not the branch-commit marker (exactly the reviewer's flagged failure mode). `qari-assets-8` cut. **Benign**: PR was CLAUDE.md/docs-only, zero asset files → -8 = byte-identical to -7; nothing pins -8 (app + build tool pin -6). LESSON for remaining release-sensitive merges: marker must be typed into the SQUASH/MERGE subject at merge time — surfaced loudly in the recap runbook |
| M-2 | merge | PR #1 umbrella CI — red gate diagnosed + fixed | ✅ | `2f8cb75` | Only failing check was doc-freshness: check-freshness.py treated the external-ref sentinel `verified_against: n/a` as a git SHA. Fixed (SHA-shaped → ancestry-checked; n/a → exempt; else → loud BAD_BASE) + normalized 2 backtick-wrapped sentinels. All 4 gates green; **PR #1 now GREEN, ready to merge (runbook #1)** |
| 3.10 | 3 | emnlabs blind verify | ✅(after repair) | `9aa12cf` | Round 1 FAIL: 41 pre-reset citations lacked the archive-tag suffix in product/ops/analytics (orchestration gap — suffix rule had only reached the history auditor). Repaired mechanically w/ per-SHA reachability classification; deterministic re-check clean. Verifier's 9 `IN_PROGRESS`-illegal flags REJECTED w/ evidence (legal in taxonomy — verifier-instance error). Past-learning #5: suffix rule is all-surface |
| 3.11 | 3 | Qari baseline tag + reviewer / emnlabs T1 fixer | ▶ (relaunching — spend limit killed both mid-run) | Qari tag `baseline-2026-08` cut | Resuming from transcripts |
| 3.12 | 3 | Qari PR opened | ✅ | **[PR #71](https://github.com/nmalick/Qari/pull/71)** | Reviewer SHIP-WITH-NOTES; tag pushed; T2-04 cold-cache CI caveat + T3 list in the PR body |
| ENV-1 | env | code-fixer isolation:worktree defect | ⚠️ recorded | — | `isolation: worktree` places the fixer's worktree in the SESSION repo (EMN_labOS); the isolation guard then hard-refuses git on a nested GITIGNORED target repo (emnlabs_site). Qari worked only because its worktree happened to land in-repo. **Workaround used: orchestrator runs the fix loop directly** (one-commit-per-item, build-gated each) — reviewer independence preserved. Applies to bil-app + art too |
...
| 3.17c | 3 | bil-app audits + fixes + verify | ✅ | `958607c`..`20d4830` + tag | 432 claims verified (1 FAIL: missing rebuild-branch suffix → fixed). **2 real bugs found & fixed**: Express route shadowing made /api/v1/teams/statistics unreachable while the client calls it; 2 files imported a non-existent export (tsc 19→17). 3 landing-page T1s build-gated; backend README's fabricated endpoint list replaced. Auditors corrected 3 of my pre-seed claims. Reviewer gate running |
| 3.18 | 3 | art_is_everywear baseline | ▶ (eng+ops done: 480 claims) | kit `185a49d`, `cb2da8c`, `cc2d998` | Engineering audit corrected **6 recon facts** incl. two live-site findings: the seed switch **fails OPEN into demo mode** and is actually TWO independent server/client switches; the demo admin cookie is client-set, no HttpOnly/Secure. **P0 fixed**: .gitignore lacked admin-credential filename patterns while a setup script tells operators to place such a file at repo root — patterns added, both forms now ignored, none present in tree. README corrected for the pivot. product/history/backlog auditor still writing | | — | full baseline from origin/main (41 commits); code stays at root |
| 3.17d | 3 | bil-app reviewer NO-SHIP → remediated → verify PASS | ✅ | `a869c02`, `b6b0018` | **Reviewer caught an orchestration error of mine**: docs audited at origin/main then code fixed in the SAME range → citations pointed at deleted files + already-fixed bugs; backlog claimed "0 3 docs-only" vs reality `0 12`. All surfaces repointed (RESOLVED banners keep pre-fix provenance), manifests annotated, verified_against bumped. Verify round 2 FAILed (annotations were in prose, not manifests) → round 3 **PASS 432/432**. Re-review running. Both past-learnings encoded in /baseline-audit |
| 3.18b | 3 | art audits complete + proactive repoint | ✅ | `cb2da8c`, `cc2d998`, `edd43e9` | 1,435 claims total (eng+ops 480, product/history/backlog 955). Applied the audit-then-fix discipline PROACTIVELY: 23 README citations remapped by content, 3 removed-line citations annotated, all 13 surfaces bumped. secrets.md's P0 marked RESOLVED. Blind verify running. **Client findings**: split-brain risk confirmed (unset flag → demo catalog + live order writes accepted); nothing notifies the studio on a new order request |
| 5.1 | 5 | Bootstrap/snapshot contract rewrite + restore test | ✅ | `f3ce4d9` | 5 fixes: --config-only help corrected (it skips the wall), restore() PRUNES (planted canary proved retired agents survived forever before), mcp-accounts never clobbered, entry points link from commands/ OR skills/, full mode dies if the wall isn't standing. **Restore test 9/9** — evidence at claude-output-docs/history/rebuild-2026-08/restore-test.md |
