...
description: Invoke at milestones (post-validation, post-design, post-build,
  ...
  issues for new docs, and (optionally) sends milestone notifications with links.
...
At each milestone: update local status, create a GitHub issue, and optionally notify via a configured channel.
...
If no git repo exists in the project directory, skip GitHub issue creation and note it in the milestone summary.

### Milestone notifications (optional)
If a notification channel is configured for this project, send the milestone summary there.
If no channel is configured, skip silently — GitHub issues remain the system of record.
Do not assume any specific provider; read the channel config from the project (e.g. `docs/status/notify.config`)
and skip if absent. Never hardcode tokens or chat IDs in this file.
...
### Step C — Send milestone notification (optional)

If a channel is configured, send this summary; otherwise skip.
Format:
```
...
4. Send summary via the configured notification channel (if any)
...
2. Send confirmation via the configured channel (if any)
...
- Every milestone → notification with issue links (if a channel is configured)
- Max 300 chars per notification message — link to GitHub issue for details
- If `gh` or the notification send fails, write error to `docs/status/errors.log` and continue
